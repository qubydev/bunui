...
export function JellyBun({
  ...
  useEffect(() => {
    ...
    const blinkEye = (
      ...
      if (isPressedRef.current || isAngryRef.current) return;
    ...
    const angryTween = angryTweenRef.current;
    const angryMarkTween = angryMarkTweenRef.current;
    ...
    const angryTimer = angryTimerRef.current;
  ...
  const triggerAnger = () => {
    ...
    angryTimerRef.current = setTimeout(() => {
      angryTimerRef.current = null;
      if (!isPressedRef.current) relaxAngryEyes();
    }, ANGRY_COOLDOWN_MS);
  };
  ...
  const press = () => {
    ...
    const leftEye = leftEyeRef.current;
    const rightEye = rightEyeRef.current;
    if (!bun || !leftEye || !rightEye) return;
  ...
  const release = () => {
    ...
    const leftEye = leftEyeRef.current;
    const rightEye = rightEyeRef.current;
    if (!bun || !leftEye || !rightEye || !isPressedRef.current) return;
